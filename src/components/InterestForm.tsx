...
export function InterestForm({ onSubmitted }: { onSubmitted: () => void }) {
  ...
  const canProceed = (() => {
    if (q.kind === "text") {
      return q.optional || String(value ?? "").trim().length > 0;
    }
  ...
  const toggleMulti = (option: string) => {
    ...
    if (i >= 0) {
      picked.splice(i, 1);
    } else if (!q.exactly || picked.length < q.exactly) {
      picked.push(option);
    }
  ...
  const submit = async () => {
    ...
    const preferredSchedule = String(
      answers["preferred_schedule"] ?? "",
    );
    const preferredTrainingTime = String(
      answers["preferred_time"] ?? "",
    );
    const membershipFit = String(
      answers["quarterly_commitment"] ?? "",
    );
    const anythingElse = String(
      answers["notes"] ?? "",
    ).trim();
    ...
    if (missingRequired) {
      setError(
        "Some answers are missing. Please go back and complete them.",
      );
      return;
    }
    ...
    if (sixMonthWin.length !== 2) {
      setError(
        "Please go back and pick exactly 2 answers for question 05.",
      );
      return;
    }
    ...
    try {
      ...
      // -----------------------------------------
      // 2. SEND EMAIL USING FORMSUBMIT
      // -----------------------------------------

      const emailResponse = await fetch(
        "https://formsubmit.co/ajax/achalpardeshi159@gmail.com",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            _subject: `New BMC interest form submission — ${name}`,
            _template: "table",
            ...
            heard_about: heardAbout,
            familiarity,
            six_month_win: sixMonthWin.join(" · "),
            patience,
            current_training: currentTraining.join(" · "),
            preferred_schedule: preferredSchedule,
            preferred_training_time: preferredTrainingTime,
            membership_fit: membershipFit,
            anything_else:
      ...
      console.log(
        "[interest] FormSubmit response:",
        emailResult,
      );

      if (!emailResponse.ok || !emailResult.success) {
        console.error(
          "[interest] FormSubmit error:",
          emailResult,
        );

        setSubmitting(false);
        setError(
          "Your details were saved, but the email notification could not be sent. Please try again.",
        );
        return;
      }
  ...
  return (
    ...
    >
      ...
      <div
        key={step}
        className="quiet-in mt-16 flex-1 sm:mt-24"
      >
